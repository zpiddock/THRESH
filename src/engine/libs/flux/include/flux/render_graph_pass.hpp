...
namespace flux {
    ...
    struct FLUX_API ColorAttachmentConfig {
    ...
    struct FLUX_API DepthAttachmentConfig {
        ResourceHandle handle = INVALID_RESOURCE;
        VkAttachmentLoadOp load_op = VK_ATTACHMENT_LOAD_OP_CLEAR;
    ...
    struct FLUX_API ResourceAccess {
        ResourceHandle handle = INVALID_RESOURCE;
        ResourceUsage usage = ResourceUsage::SampledImage;
        VkPipelineStageFlags2 stage_override = 0; // 0 = use default from usage

        // Descriptor binding info (for auto-generated descriptor sets)
        // If binding is ~0u, this access won't be included in auto-generated descriptors
        uint32_t binding = ~0u;
        VkSampler sampler = VK_NULL_HANDLE;  // Required for SampledImage usage
    };

    // Pass configuration
    struct FLUX_API PassConfig {
        std::string name;
        ...
        // Auto-descriptor generation
        // If true, a descriptor set will be generated from reads/writes that have binding != ~0u
    ...
    struct FLUX_API PassExecutionContext {
        ...
        [[nodiscard]] auto has_pass_descriptors() const -> bool {
            return pass_descriptor_set != VK_NULL_HANDLE;
        }
    };
    ...
    struct FLUX_API PassDefinition {
        PassConfig config;
    ...
    FLUX_API auto compute_resource_state(
