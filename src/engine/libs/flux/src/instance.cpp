...
namespace flux {
    ...
    VKAPI_ATTR VkBool32 VKAPI_CALL debug_callback(
            ...
            switch (message_severity) {
                case VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT:
                    SUB_TRACE("[Vulkan] {}", callback_data->pMessage);
                    break;
                case VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT:
                    SUB_INFO("[Vulkan] {}", callback_data->pMessage);
                    break;
                case VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT:
                    SUB_WARN("[Vulkan] {}", callback_data->pMessage);
                    break;
                case VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT:
                    SUB_ERROR("[Vulkan] {}", callback_data->pMessage);
                    break;
                default:
                    SUB_DEBUG("[Vulkan] {}", callback_data->pMessage);
                    break;
            }
    ...
    Instance::Instance(const Config &config) : m_validation_enabled(config.enable_validation) {
        ...
        if (m_validation_enabled && !config.validation_layers.empty()) {
            SUB_DEBUG("Enabling {} validation layers", config.validation_layers.size());
        ...
        if (::vkCreateInstance(&create_info, nullptr, &m_instance) != VK_SUCCESS) {
            SUB_FATAL("Failed to create Vulkan instance");
            throw std::runtime_error("Failed to create Vulkan instance");
        }

        SUB_DEBUG("Vulkan instance created successfully");
    ...
    Instance::~Instance() {
        ...
        if (m_instance != VK_NULL_HANDLE) {
            SUB_DEBUG("Destroying Vulkan instance");
            ::vkDestroyInstance(m_instance, nullptr);
            SUB_DEBUG("Vulkan instance destroyed successfully");
        }
    }
    ...
    auto Instance::setup_debug_messenger() -> void {
        SUB_DEBUG("Setting up Vulkan debug messenger");
        ...
        if (func && func(m_instance, &create_info, nullptr, &m_debug_messenger) != VK_SUCCESS) {
            SUB_ERROR("Failed to set up debug messenger");
