#include "flux/pipeline.hpp"
#include "flux/shader.hpp"
...
namespace flux {
    Pipeline::Pipeline(const Config &config) : m_device(config.device), m_config(config) {
        SUB_INFO("Creating graphics pipeline with {} raw shader stages and {} shader objects",
        ...
        if (::vkCreatePipelineLayout(m_device, &pipeline_layout_info, nullptr, &m_pipeline_layout) != VK_SUCCESS) {
            SUB_FATAL("Failed to create pipeline layout");
            throw std::runtime_error("Failed to create pipeline layout");
        }

        create_pipeline();
        SUB_DEBUG("Graphics pipeline created successfully");
    }

    Pipeline::~Pipeline() {
        SUB_DEBUG("Destroying graphics pipeline");
        cleanup();

        if (m_pipeline_layout != VK_NULL_HANDLE) {
            ::vkDestroyPipelineLayout(m_device, m_pipeline_layout, nullptr);
        }
        SUB_DEBUG("Graphics pipeline destroyed successfully");
    }
    ...
    auto Pipeline::reload(const std::vector<ShaderStage> &new_shader_stages) -> void {
        SUB_INFO("Hot-reloading pipeline with {} new shader stages", new_shader_stages.size());
        ...
        SUB_INFO("Pipeline hot-reloaded successfully");
    }

    auto Pipeline::load_shader_from_file(const std::string &filepath) -> std::vector<std::uint32_t> {
        SUB_DEBUG("Loading shader from file: {}", filepath);

        std::ifstream file(filepath, std::ios::ate | std::ios::binary);

        if (!file.is_open()) {
            SUB_ERROR("Failed to open shader file: {}", filepath);
