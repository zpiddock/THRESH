...
namespace flux {
    Shader::Shader(const Config &config)
        ...
        try {
            ...
            SUB_INFO("Loaded shader: {}", m_filepath.string());
        } catch (const std::exception& e) {
            m_last_error = e.what();
            if (m_throw_on_error) {
                throw;
            }
            SUB_ERROR("Shader compilation failed for '{}': {}", m_filepath.string(), m_last_error);
        }
    }
    ...
    auto Shader::reload() -> bool {
        try {
            SUB_INFO("Reloading shader: {}", m_filepath.string());
            ...
            if (::vkCreateShaderModule(m_device, &create_info, nullptr, &new_module) != VK_SUCCESS) {
                SUB_ERROR("Failed to reload shader: {}", m_filepath.string());
                return false;
            }
            ...
            SUB_INFO("Successfully reloaded shader: {}", m_filepath.string());
            ...
            SUB_ERROR("Exception while reloading shader: {}", e.what());
            return false;
        }
    }
