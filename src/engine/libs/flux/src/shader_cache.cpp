...
namespace flux {
    ...
    ShaderCache::ShaderCache(const Config &config)
        ...
        if (!std::filesystem::exists(m_cache_dir)) {
            try {
                ...
                SUB_INFO("Created shader cache directory: {}", m_cache_dir.string());
            } catch (const std::exception &e) {
                SUB_ERROR("Failed to create cache directory: {}", e.what());
            }
        }
    }
    ...
    auto ShaderCache::lookup(const std::filesystem::path &source_path) -> std::optional<std::vector<std::uint32_t> > {
        ...
        if (!std::filesystem::exists(cache_path)) {
            SUB_DEBUG("Cache miss for {}: no cache file", source_path.string());
            return std::nullopt;
        }

        auto entry = load_cache_entry(cache_path);
        if (!entry) {
            SUB_DEBUG("Cache miss for {}: failed to load", source_path.string());
            return std::nullopt;
        }

        if (m_config.enable_validation && !is_cache_valid(source_path, *entry)) {
            SUB_DEBUG("Cache miss for {}: validation failed", source_path.string());
            return std::nullopt;
        }

        SUB_INFO("Cache hit for {}", source_path.string());
        return entry->spirv;
    }
    ...
    auto ShaderCache::store(const std::filesystem::path &source_path,
        ...
        if (!std::filesystem::exists(source_path)) {
            SUB_WARN("Cannot cache shader: source file does not exist: {}", source_path.string());
            return;
        }
        ...
        SUB_DEBUG("Cached shader: {} -> {}", source_path.string(), cache_path.string());
    }
    ...
    auto ShaderCache::clear() -> void {
        ...
        try {
            ...
            SUB_INFO("Cleared {} cached shaders", removed);
        } catch (const std::exception &e) {
            SUB_ERROR("Failed to clear cache: {}", e.what());
        }
    }
    ...
    auto ShaderCache::prune() -> void {
        ...
        try {
            ...
            SUB_INFO("Pruned {} stale cache entries", pruned);
        } catch (const std::exception &e) {
            SUB_ERROR("Failed to prune cache: {}", e.what());
        }
    }
    ...
    auto ShaderCache::is_cache_valid(const std::filesystem::path &source_path,
        ...
        if (current_timestamp > entry.timestamp) {
            SUB_DEBUG("Cache invalid: source file modified");
            return false;
        }
        ...
        if (current_hash != entry.source_hash) {
            SUB_DEBUG("Cache invalid: source hash mismatch");
            return false;
        }
    ...
    auto ShaderCache::load_cache_entry(const std::filesystem::path &cache_path) -> std::optional<CacheEntry> {
        ...
        if (magic != CACHE_MAGIC || version != CACHE_VERSION) {
            SUB_WARN("Invalid cache file: {}", cache_path.string());
            return std::nullopt;
        }
        ...
        if (!file) {
            SUB_WARN("Failed to read cache file: {}", cache_path.string());
            return std::nullopt;
        }
    ...
    auto ShaderCache::save_cache_entry(const std::filesystem::path &cache_path,
        ...
        if (!file.is_open()) {
            SUB_ERROR("Failed to write cache file: {}", cache_path.string());
            return;
        }
