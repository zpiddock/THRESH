...
namespace flux {
    ...
    auto ShaderCompiler::compile(const std::string &source,
        ...
        // Convert stage
        auto stage = static_cast<EShLanguage>(stage_to_glslang_stage(options.stage));
        ...
        // Parse
        SUB_DEBUG("Compiling shader: {}", source_name);
        ...
        if (!shader.parse(&default_builtin_resources, default_version, false, messages)) {
            ...
            SUB_ERROR("Shader compilation failed for {}: {}", source_name, result.error_message);
            return result;
        }
        ...
        if (!program.link(messages)) {
            ...
            SUB_ERROR("Shader linking failed for {}: {}", source_name, result.error_message);
            return result;
        }

        // Get warnings
        if (shader.getInfoLog() && strlen(shader.getInfoLog()) > 0) {
            result.warnings.emplace_back(shader.getInfoLog());
            SUB_WARN("Shader compilation warnings for {}: {}", source_name, shader.getInfoLog());
        }
        ...
        if (!logger.getAllMessages().empty()) {
            SUB_WARN("SPIR-V generation messages: {}", logger.getAllMessages());
        }

        result.success = true;
        SUB_DEBUG("Successfully compiled {} to SPIR-V ({} bytes)",
