...
namespace flux {
    Surface::Surface(VkInstance instance, horizon::Window &window)
        : m_instance(instance) {
        SUB_DEBUG("Creating Vulkan surface");

        // TODO: Fuck this off and have the horizon::Window create the surface
        ...
        if (result != VK_SUCCESS) {
            SUB_ERROR("Failed to create window surface");
            throw std::runtime_error("Failed to create window surface");
        }

        SUB_DEBUG("Vulkan surface created successfully");
    }

    Surface::~Surface() {
        if (m_surface != VK_NULL_HANDLE) {
            SUB_DEBUG("Destroying Vulkan surface");
            ::vkDestroySurfaceKHR(m_instance, m_surface, nullptr);
            SUB_DEBUG("Vulkan surface destroyed");
        }
    }
