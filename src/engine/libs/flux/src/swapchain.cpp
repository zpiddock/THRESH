...
namespace flux {
    Swapchain::Swapchain(const Config &config) : m_device(config.device), m_config(config) {
        SUB_INFO("Creating swapchain ({}x{})", config.width, config.height);
        create_swapchain();
        create_image_views();
        SUB_DEBUG("Swapchain created with {} images", m_images.size());
    }

    Swapchain::~Swapchain() {
        SUB_DEBUG("Destroying Swapchain");
        cleanup();
    }
    ...
    auto Swapchain::resize(std::uint32_t width, std::uint32_t height) -> void {
        SUB_INFO("Resizing swapchain from {}x{} to {}x{}", m_config.width, m_config.height, width, height);
        ...
        SUB_DEBUG("Swapchain resized successfully");
    }
    ...
    auto Swapchain::create_swapchain() -> void {
        ...
        if (support_details.capabilities.maxImageCount > 0 && image_count > support_details.capabilities.
            maxImageCount) {
    ...
    auto Swapchain::cleanup() -> void {
        ...
        if (m_swapchain != VK_NULL_HANDLE) {
            ...
            SUB_DEBUG("Destroyed Swapchain");
        }
    }

    auto Swapchain::choose_surface_format(
