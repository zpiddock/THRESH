...
Semaphore::Semaphore(const Config& config)
    ...
    SUB_TRACE("Created VkSemaphore: {}", static_cast<void*>(m_semaphore));
}

Semaphore::~Semaphore() {
    if (m_semaphore != VK_NULL_HANDLE && m_device != VK_NULL_HANDLE) {
        SUB_TRACE("Destroying VkSemaphore: {}", static_cast<void*>(m_semaphore));
...
Fence::Fence(const Config& config)
    ...
    SUB_TRACE("Created VkFence: {} (signaled: {})",
...
Fence::~Fence() {
    if (m_fence != VK_NULL_HANDLE && m_device != VK_NULL_HANDLE) {
        SUB_TRACE("Destroying VkFence: {}", static_cast<void*>(m_fence));
