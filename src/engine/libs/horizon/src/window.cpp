...
#include "../../substratum/include/substratum/log.hpp"
#include <stdexcept>

namespace horizon {
    Window::Window(const Config &config) : m_config(config) {
        SUB_INFO("Initializing GLFW");

        if (!::glfwInit()) {
            SUB_FATAL("Failed to initialize GLFW");
            throw std::runtime_error("Failed to initialize GLFW");
        }

        SUB_INFO("Creating window: \"{}\" ({}x{})", config.title, config.width, config.height);
        ...
        if (!m_window) {
            ::glfwTerminate();
            SUB_FATAL("Failed to create GLFW window");
            throw std::runtime_error("Failed to create GLFW window");
        }

        SUB_DEBUG("Window created successfully");
    }

    Window::~Window() {
        if (m_window) {
            SUB_DEBUG("Destroying window");
            ::glfwDestroyWindow(m_window);
            ::glfwTerminate();
            SUB_DEBUG("Window successfully");
        }
    }
